...
std_msgs::Int16 right_wheel_velo;
ros::Publisher veloPub("right_velo", &right_wheel_velo);
...
const int TICKS_PER_REVOLUTION = 31250;
 
// Wheel radius in meters
const double WHEEL_RADIUS = 0.09525;
 
// Distance from center of the left tire to the center of the right tire in m
const double WHEEL_BASE = 0.6858;
//0.59847 meters per revolution **linear distance per wheel rev** 
// Number of ticks a wheel makes moving a linear distance of 1 meter
// This value was measured manually.
const double TICKS_PER_METER = 52217; // Originally 2880
...
const int DAC_TURN = 1000;
 
// Set maximum and minimum limits for the PWM values
const int DAC_MIN = 800; // about 0.1 m/s
const int DAC_MAX = 1400; // about 0.172 m/s
 
// Set linear velocity and PWM variable values for each wheel
double velLeftWheel = 0;
double velRightWheel = 0;
double dacLeftReq = 0;
double dacRightReq = 0;
...
void calc_vel_right_wheel(){
  ...
  right_wheel_velo.data = numOfTicks;
...
void calc_pwm_values(const geometry_msgs::Twist& cmdVel) {
  ...
  dacRightReq = K_P * cmdVel.linear.x + b;
  ...
  // Go straight
    ...
    dacRightReq += (int)(avgDifference * DRIFT_MULTIPLIER);
  }
 
  // Handle low PWM values
  if (abs(dacLeftReq) < DAC_MIN) {
    dacLeftReq = 0;
  }
  if (abs(dacRightReq) < DAC_MIN) {
    dacRightReq = 0;  
  }  
}
...
void set_pwm_values() {/*
  ...
  */
  myDac.analogWrite(0,0,0,1,1000);
  myDac.analogWrite(1,0,0,1,1000);
}
...
void loop() {
  ...
  if (currentMillis - previousMillis > interval) {
    ...
    veloPub.publish( &right_wheel_velo );
